...
# x축, y축, b=“포인트+연결라인”
plot(dose, drugA, type="b")    # A약 투여량과 반응을 표현한 간단한 라인 그래프
...
opar <- par(no.readonly=TRUE)   #그래프 모수를 설정할 때는 나중에 원래 그래프 모수로 돌아와야하는 상황에서 편리하게 사용하기 위해 사용
par(lty=4, pch=16)
...
plot(dose, drugA, type="b",
     ...
     main="Clinical Trials for Drug A",     #main: 본제목
     sub="This is hypothetical data",       #sub : 부제목
     xlab="Dosage", ylab="Drug Response",   #xlab : x축 이름, ylab : y축 이름
     xlim=c(0, 60), ylim=c(0, 70))          #xlim : x축 범위, ylim : y축 범위
...
plot(dose, drugA, type="b",
     col="red", lty=2, pch=2, lwd=2,
     main="Clinical Trials for Drug A",col.main="red",       #col.main : 본제목 글자의 색변경
     sub="This is hypothetical data",col.sub="blue",         #col.sub : 부제목 글자의 색변경
     xlab="Dosage", ylab="Drug Response",col.lab="green", cex.lab=0.75,  #col.lab : x, y축 라벨 글자의 색변경
...
plot(x, y, type="b", pch=21, col="red",ylab="",yaxt="n",lty=3)  
...
text(5,6,"Example of default text")
...
barplot(counts,
        main="가로 Bar Plot",
        xlab="빈도수", ylab="약효 구분",
        horiz=TRUE)             #horiz=TRUE는 가로로 Bar Plot를 그림
...
barplot(counts2,
        ...
        xlab="처방구분", ylab="빈도수",
        col=c("red", "yellow", "green"),
        legend=rownames(counts), beside=TRUE )   # beside=TRUE로 막대그래프를 나누어서 볼 수 있음
...
x11()
n <- 10
mycolors <- heat.colors(n)       #heat.colors : rgb컬러를 만들어주는 함수
barplot(counts2,
        main="Grouped Bar Plot",
        xlab="처방구분", ylab="빈도수",
        col=mycolors,
...
mean_f <- aggregate(states$Illiteracy, by=list(state.region), FUN=mean)
#- 집계함수(aggregate)로 states의 Illiteracy의 값을 state.region별로 평균을 구함
mean_f

# 정렬하기
mean_f <- mean_f[order(mean_f$x), ]  #order 함수로 정렬, decreasing=T이면 내림차순으로 정렬렬
mean_f
(mean_f[,2])
...
counts <- table(Arthritis$Improved)
counts
par(mar=c(5,8,4,2))  # 여백을 수정
par(las=2)           # 축의 텍스트를 90도 회전
barplot(counts,   main="처방 결과표",   horiz=TRUE, cex.names=0.8,
...
# 기본 파이 차트 그리기
pie( slices, labels = lbls, main="기본 파이 차트")
...
pie(slices, labels=lbls2, col=rainbow(length(lbls2)), main="파이 차트(비율)")

# 3차원 파이 차트 그리기
library( plotrix )
pie3D(slices, labels=lbls2, explode=1.2,	main="3D 파이 차트 ")

# 라벨 변경하기 : 카테고리 이름 + 빈도수
mytable <- table(state.region)
lbls3 <- paste(names(mytable), "\n", mytable, sep="") 
pie(mytable, labels=lbls3, 	main="파이 차트(빈도수)")
...
boxplot(mpg ~ cyl, data=mtcars,
        notch=TRUE,        #notch를 통해 박스의 가위집을 냄
        varwidth=TRUE,     #varwidth=TRUE를 통해 관측치 수의 비율에 맞는 너비로 그려짐
        col=heat.colors(5),
        main="차량 연비 비교",
        xlab="실린더 수",
        ylab="Miles/Gallon")
...
boxplot(mpg ~ am.f * cyl.f,          #mpg 값을 am.f와 cyl.f 변수에 따라 박스 그래프 생성
        data=mtcars,
        varwidth=TRUE,
...
x1 <- mtcars$mpg[mtcars$cyl==4]    #cyl이 4인 mpg 값을 x1에 저장
x2 <- mtcars$mpg[mtcars$cyl==6]    #cyl이 6인 mpg 값을 x2에 저장
x3 <- mtcars$mpg[mtcars$cyl==8]    #cyl이 8인 mpg 값을 x3에 저장
...
x <- mtcars[order(mtcars$mpg), ]  # mpg값을 기준으로 정렬
x
...
dotchart( x$mpg,
          labels = row.names(x),	
          cex=.7,
          groups = x$cyl,             # 4기통, 6기통, 8기통 그룹에 따른 값을 나타냄
          gcolor = "black",
