...
#귀무가설 : 서울 지역 고등학교 1학년 남학생의 몸무게는 5년 전보다 증가하지 않음
#대립가설 : 서울 지역 고등학교 1학년 남학생의 몸무게가 5년 전보다 더욱 증가
...
# 귀무가설 : 모집단은 정규분포를 따른다.
# 대립가설 : 모집단은 정규분포를 따르지 않는다.
# 유의확률(p-value)는 0.2413로 귀무가설을 채택한다.
...
t.test(x0,x1,mu=2,alternative = "two.sided", paired = T)
...
#귀무가설 : 두 부서의 직무만족도 모평균은 차이가 없다.
#대립가설 : 두 부서의 직무만족도 모평균은 차이가 있다.
...
#귀무가설 : 온도의 조건에 따라서 휘발유 생산량에 차이가 없다.
#대립가설 : 온도의 조건에 따라서 휘발유 생산량에 차이가 있다.
...
comparison <- scheffe.test(result, 
                           "group",
                           alpha = 0.05,
                           console=TRUE,
)
...
cor.test(A_doc,B_doc, alternative="two.sided", method="spearman")
