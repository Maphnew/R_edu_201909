...
is.na(airquality)
apply(is.na(airquality),2,sum)	#apply 함수를 통해 열별 결측치의 개수를 확인(TRUE는 1이므로)
...
airquality[150, "Ozone"] <- 28	#airquality 데이터의 150번째 행에서 Ozone 변수값에 28을 대체값으로 대입
...
mean(airquality$Ozone,na.rm=FALSE)	#Ozone의 데이터에 NA가 있어 평균을 구하면 NA가 나타남
mean(airquality$Ozone,na.rm=TRUE)	#Ozone의 데이터에서 NA 값을 제외한 값들의 평균 값을 구함
...
a$na<-ifelse(is.na(a$Ozone)==TRUE | is.na(a$Solar.R)==TRUE,1,0)	#Ozone과 Solar.R 중 NA가 하나라도 있으면 1, 아니면 0

sum(a$na)		#Ozone과 Solar.R 중 하나라도 NA가 있는 행은 42개
...
which(!complete.cases(airquality))			#NA가 있는 행의 번호를 출력

knnImputation(airquality[1:4],k=3)[which(!complete.cases(airquality)),]	#knnImputation을 통해 결측값 대치
...
centralImputation(airquality)[which(!complete.cases(airquality)),]	#중앙값으로 대치되어있음을 확인
...
a<-c(-10,x,19,28,30)		#임의 값을 넣음
outwith<-boxplot(a)		#boxplot을 그렸을 때, 점으로 나타나 이상치가 나타남
outwith$out				#boxplot의 점을 보여줌
