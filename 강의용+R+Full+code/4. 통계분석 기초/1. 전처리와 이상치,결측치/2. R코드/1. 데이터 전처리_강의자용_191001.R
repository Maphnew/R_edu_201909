...
str(iris)        # iris 데이터의 구조 확인 : 150개의 행과 5개의 변수를 가지고 있음
iris$ID<-1:150   # 150개의 개체의 고유번호에 해당하는 'ID' 변수를 새로 생성
head(iris)       # head 함수로 iris 데이터의 상위 6개 행을 출력해 ID 변수가 추가된 것을 확인
...
iris["Group"]<-ifelse(iris$ID%%2==0,"A","B") # ifelse 함수를 이용하여 iD가 짝수이면 A, 홀수이면 B를 부여

head(iris)                                   # iris 데이터의 상위 6개의 행을 출력 : Group 변수가 추가된 것을 확인할 수 있음
...
transform(iris, Sum.Length=Sepal.Length + Petal.Length) #sum.Length라는 변수 생성
...
score_df <- within( score_df, {
  grade = character(0)          # 새로운 변수 (생략하여도 상관 없음)
...
mutate_2<-mutate(mutate_1,						#mutateg함수를 통해 생성된 변수를 바로 활용해 신규 변수 생성가능 
...
head(Fruits)
Fruits %>%
  group_by(Fruit) %>% 					#Fruits데이터 셋을 받아 Fruit별로 묶기 
  summarize(sales_mean = mean(Sales, na.rm=T))	#묶은 Fruit를 Sales를 기준으로 결측치를 제거한후에 평균을 구하기
