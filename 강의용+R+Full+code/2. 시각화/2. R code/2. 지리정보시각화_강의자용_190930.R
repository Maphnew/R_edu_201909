...
#- unique : 중복이 없는 유일한 관측치를 선별할 때 사용하는 함수
...
map('worldHires', region = c('South Korea'), col = 'blue', add = T, fill = T)
...
map <- get_googlemap("서울,대한민국", zoom=14, maptype="roadmap",
...
kor.map <- ggmap(map) + geom_point(data=loc1, aes(x=LON, y=LAT), color="red",  size=4, alpha=0.7) +
...
ggmap(kor)
kor.map <- ggmap(kor) + geom_point(data=loc1, aes(x=LON, y=LAT), size=3, color="green")+
  geom_point(data=loc2, aes(x=LON, y=LAT), size=3, color="red")

kor.map


kor.map + geom_text(data=loc1, aes(x = LON, y = LAT+0.005, label=역명), size=3) +
